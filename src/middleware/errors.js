...
function errorHandler(err, _req, res, _next) {
  ...
  if (err && (err.code === 'DATABASE_CONNECTION_ERROR' || err.name === 'MongoServerSelectionError' || err.name === 'MongoNetworkError' || /buffering timed out|database connection/i.test(err.message))) return res.status(503).json({ error: err.publicMessage || 'Database connection error.', details: null });
