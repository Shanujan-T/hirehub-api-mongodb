...
const Community = mongoose.model('Community', schema({ admin_id: Number, name: { type: String, required: true, unique: true }, description: String, category_id: { type:Number, required:true }, location: String, image_url: String, experience_level:{type:String,enum:['less_than_1_year','1_to_3_years','3_plus_years']},specialization:String,portfolio_links:{type:[String],default:[]},admin_bio:String,contact_phone:String,status: { type: String, enum:['pending','approved','rejected'],default: 'pending' }, rejection_reason:String, verification_status: String, reputation_score: { type: Number, default: 0 }, weekly_digest: String }, 'communities'));
...
const CommunityApplication = mongoose.model('CommunityApplication', schema({ job_id: {type:Number,required:true}, community_id: {type:Number,required:true}, applied_by_id: Number, status: { type: String, enum:['applied','approved','rejected'],default: 'applied' }, source: { type: String, enum:['applied','invited'],default: 'applied' }, proposed_cost:{type:Number,required:true},proposed_days:{type:Number,required:true},note:String,bid_amount:Number,proposal:String,commission_percent:Number,applied_at:{type:Date,default:Date.now} }, 'community_applications', [[{ job_id: 1, community_id: 1 }, { unique: true }]]));
const Contract = mongoose.model('Contract', schema({ job_id: {type:Number,required:true,unique:true}, community_id: {type:Number,required:true}, assigned_member_id: Number, status: { type: String, enum:['pending_assignment','open_internally','active','submitted','completed','disputed'],default: 'pending_assignment' }, total_amount:{type:Number,required:true},commission_percent:{type:Number,default:3},commission_amount:Number,member_payout:Number,agreed_price:Number, deliverable_url: String, deliverable_notes: String, submitted_at: Date, admin_approved_at: Date, poster_approved_at: Date, risk_level:{type:String,enum:['none','low','high'],default:'none'},risk_reason:String,risk_flags:{type:[String],default:[]},risk_checked_at:Date,health_status: String, health_reason: String, health_scored_at: Date }, 'contracts'));
